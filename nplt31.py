...
class NpltConfig:
    ...
    full_tree_mode: bool = False
    web_page_color_analysis: bool = True
    keyword_list: int = 50
    min_link_limit: int = 5
    skip_word_mode: bool = True
    case_sensitive_mode: bool = False
    debug_mode: bool = False
    report_lang: int = 2
    db_connection: int = 0
    db_connection_id: str = "0"
    stop_line: int = 0
    year_url_skip: bool = True
    forbidden_word_enable: bool = False
    search_string: str = ""
    keyword_path: str = "/"
    file_download: list = field(default_factory=list)
    respect_robots: bool = True
    max_pages: int = 3000

    @staticmethod
    def _matches(value, choices):
        return str(value).upper() in choices
    ...
    def from_args(cls, args):
        return cls(
            cost_saving_mode=not cls._matches(args.cost, ["NO", "N", "0"]),
            full_tree_mode=cls._matches(args.tree, ["FULL", "F", "0"]),
            web_page_color_analysis=not cls._matches(args.ca, ["NO", "F", "0"]),
            keyword_list=cls._int_or_default(args.wl, 50),
            min_link_limit=cls._int_or_default(args.ml, 5),
            skip_word_mode=not cls._matches(args.sw, ["NO", "F", "0"]),
            case_sensitive_mode=not cls._matches(args.cs, ["NO", "F", "0"]),
            debug_mode=cls._matches(args.dm, ["YES", "Y", "1"]),
            report_lang=2 if cls._matches(args.rl, ["EN"]) else 1,
            db_connection=1 if cls._matches(args.db, ["YES", "Y", "1", "TRUE"]) else 0,
            db_connection_id=str(args.id),
            stop_line=cls._int_or_default(args.sl, 0),
            year_url_skip=cls._matches(args.ys, ["YES", "Y", "1"]),
            forbidden_word_enable=cls._matches(args.fw, ["YES", "Y", "1"]),
            search_string=args.ss if len(args.ss) > 1 else "",
            keyword_path=args.wp if len(args.wp) > 1 else "/",
            file_download=[] if cls._matches(args.fd, ["NO"]) else Extract_extion_fd(args.fd),
            respect_robots=not cls._matches(args.robots, ["NO", "N", "0"]),
            max_pages=cls._int_or_default(args.mp, 3000),
        )
...
class NpltScanner:
    def __init__(self, config):
        self.config = config
        self.state = NpltState()

    def prepare_target(self, url):
    ...
    def add_scan_list(self, t, f, u, source=None):
        ...
        before_count = len(self.state.scan_web_set)
        self.state.scan_web_set.add(t)
        if len(self.state.scan_web_set) > before_count:
            self.state.scan_web_list.append([t, f, u])

        scanWebList = self.state.scan_web_list
        scanWebSet = self.state.scan_web_set

    def add_graph_node(self, tag, url):
        global G, visitLinkDict
        ...
        if tag not in self.state.graph:
            ...
            self.state.visit_link_dict[tag] = 0    ###  0>1>0 2019.11.2

        if not self.state.graph.has_edge(url, tag):
            self.state.graph.add_edge(url, tag)
    ...
    def sync_from_legacy_globals(self):
        ...
        global AddFavoriteCount, AddFavoriteSet
        global extion_count, word_count, list_title, list_flash, list_plugin
    ...
    def apply_config_to_legacy_globals(self):
        ...
        webPageColorAnalysis = self.config.web_page_color_analysis
        keyWordList = self.config.keyword_list
        MinLinkLimit = self.config.min_link_limit
        skipWordMode = self.config.skip_word_mode
        caseSensitiveMode = self.config.case_sensitive_mode
        Debug_mode = self.config.debug_mode
        Search_string = self.config.search_string
        rptLang = self.config.report_lang
        forbiddenWordEnable = self.config.forbidden_word_enable
        dbConnection = self.config.db_connection
        dbConnection_id = self.config.db_connection_id
        StopLine = self.config.stop_line
        yearUrlskip = self.config.year_url_skip
...
MessageList=[
                ...
                [55, "Google 존재 여부", "Is this URL indexed by Google?" ,"google"],
...
def normalized_edges_from_graph(graph):
    for source, target in graph.edges():
        yield modi_url(source), modi_url(target)
...
def google_site_exists(domain, num=10):
    ...
    try:
        ...
        for a in soup.find_all("a", href=True):
            ...
            # 1) href 자체에 domain이 포함되어 있는 경우
            if domain in href:
                return 1

            # 2) Google redirect 링크: /url?q=https://www.hd.com/...
            if href.startswith("/url?"):
                parsed = urlparse(href)
                query_params = parse_qs(parsed.query)

                real_url = query_params.get("q", [""])[0]
...
def addgraphNode(tag,url):
    if CURRENT_SCANNER is not None:
        return CURRENT_SCANNER.add_graph_node(tag, url)
...
def addScanList(t,f,u,s):
    if CURRENT_SCANNER is not None:
        return CURRENT_SCANNER.add_scan_list(t, f, u, s)
...
def main():
    ...
    parser.add_argument(
        '-mp',
        help='maximum pages to scan [number]',
        default='3000',
    )
    ...
    scanner.state.list_search_string = list_Search_string
    print(">>>>>>>>", list_Search_string)

    url = scanner.prepare_target(url)
    ...
    configure_robots(url, respect=config.respect_robots)
    ...
    xt = google_site_exists(baseUrl)
    print(xt)
    exists_text = "Yes" if xt else "No"
    progress_make(1, "Exist on Google: ", exists_text)
