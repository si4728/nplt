...
def scanWeb(url, Purl):
    ...
    page_tags = collect_page_tags(content)
    #print("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&")
   ...
   # cmp_tags = merge_dicts_as_tuples(start_tag, end_tag)  
    ...
    #print("(0)",content)
    for slink in page_tags["scripts"]:
    ...
    if counter == 1:
        first_page_css = "\n".join(
            ...
            for style in page_tags["styles"]
        )
        first_page_css += "\n" + "\n".join(
            tag.get("style", "")
            for tag in page_tags["styled"]
        )
        first_page_css += "\n" + "\n".join(
            f"color:{tag.get('color')};"
            for tag in page_tags["colored"]
        )
        merge_color_counts(
            font_color_count,
    ...
    for frameset in page_tags["links"]:  #rel="stylesheet" 
    ...
    scriptCount = scriptCount + len(page_tags["scripts"])
    ...
    ######################################################
    for imgTag in page_tags["amp_imgs"]:
        imgCount = imgCount + 1
        if len(imgTag.get('alt','')) > 0:
            imgAlt = imgAlt + 1
    ...
    Debug_w(url + " >>> frame tag")
    tmp_frameset = page_tags["frames"]
    frameCount = frameCount + len(tmp_frameset)

    tmp_frameset = page_tags["iframes"]
    iframeCount = iframeCount + len(tmp_frameset)
    ##print(content)
    #######################################################
    #global baseUrl
    for frameset in page_tags["frames"]:
        tag = frameset.get("src")    
        if tag=="" or tag == None:  ## include or None  2020.2.2
            continue  
    ...
    varList=set()
    for ilink in page_tags["scripts"]:  ##.find("location.href")
        slink = str(ilink)
    ...
    #######################################################
    for link in page_tags["objects"]:
        list_plugin.append(link)
    for link in page_tags["embeds"]:
        list_plugin.append(link)
    #######################################################
    ...
    for link in page_tags["anchors"]:  ##<a class="slider" onclick="location.href='/business/field01.asp'">
