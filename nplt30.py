...
BROKEN_LINK_STATUS_CODES = {400, 403, 404, 408, 409, 501, 502, 503}
BROKEN_LINK_RETRY_COUNT = int(os.getenv("NPLT_BROKEN_LINK_RETRY_COUNT", "2"))
BROKEN_LINK_RETRY_DELAY = float(os.getenv("NPLT_BROKEN_LINK_RETRY_DELAY", "2"))
BROKEN_LINK_VERIFY_TIMEOUT = (
    10,
    int(os.getenv("NPLT_BROKEN_LINK_VERIFY_TIMEOUT", str(max(timeout_duration, 180)))),
)
...
MessageList=[
                ...
                [91, "보안 헤더 점검 및 개선 권고", "Security Header Recommendations", ""],
                [92, "    현재값: ", "    Current: ", ""],
                [93, "    권장값: ", "    Recommended: ", ""],
                [94, "    위험: ", "    Risk: ", ""],
                [95, "    조치: ", "    Action: ", ""],
                ...
                [99, "중간", "Medium", ""],
                [100, "조건부", "Conditional", ""],
                [101, "낮음/레거시", "Low/Legacy", ""],
                [102, "정보", "Info", ""],
                [103, "강한 Content-Security-Policy 적용을 우선 권장합니다. 레거시 브라우저 지원 정책이 필요할 때만 '0' 또는 '1; mode=block' 적용을 검토하십시오.", "Prefer a strong Content-Security-Policy. If legacy support is required, use '0' or '1; mode=block' by policy.", ""],
                [104, "이 헤더는 최신 브라우저에서 권장되지 않으며, XSS 방어의 주 수단이 아닙니다.", "This header is deprecated and not the primary XSS defense in modern browsers.", ""],
                [105, "주요 조치로 보지 말고 CSP를 우선 적용하십시오. 필요 시 레거시 브라우저 정책용으로만 X-XSS-Protection을 추가하십시오.", "Do not treat this as the main fix. Prioritize CSP; optionally add X-XSS-Protection for legacy browser policy only.", ""],
                [106, "의도된 설정인지 확인하고, 강한 CSP가 함께 적용되어 있는지 점검하십시오.", "Confirm this is intentional and backed by a strong CSP.", ""],
                [107, "레거시 XSS 필터링이 비활성화되어 있습니다. 일반적으로 CSP가 충분히 구성된 경우에만 허용 가능한 설정입니다.", "Legacy XSS filtering is disabled. This is usually acceptable only when CSP is configured.", ""],
                [108, "Content-Security-Policy가 존재하고 충분히 제한적인지 확인하십시오.", "Check whether Content-Security-Policy is present and sufficiently restrictive.", ""],
                [109, "브라우저가 MIME 타입을 임의로 추론하여 콘텐츠를 의도하지 않은 방식으로 실행할 수 있습니다.", "Browsers may infer a different MIME type and execute content in an unintended context.", ""],
                [110, "Nginx/OpenResty 설정에 추가: add_header X-Content-Type-Options \"nosniff\" always;", "Nginx/OpenResty: add_header X-Content-Type-Options \"nosniff\" always;", ""],
                [111, "SAMEORIGIN 또는 DENY 적용. 가능하면 CSP frame-ancestors도 함께 적용", "SAMEORIGIN or DENY; preferably also CSP frame-ancestors", ""],
                [112, "다른 사이트가 해당 페이지를 frame/iframe으로 삽입할 수 있어 클릭재킹 위험이 증가합니다.", "Other sites may embed the page in a frame, increasing clickjacking risk.", ""],
                [113, "Nginx/OpenResty 설정에 추가: add_header X-Frame-Options \"SAMEORIGIN\" always;", "Nginx/OpenResty: add_header X-Frame-Options \"SAMEORIGIN\" always;", ""],
                [114, "일반적이지 않은 X-Frame-Options 값은 클릭재킹 방어를 안정적으로 제공하지 못할 수 있습니다.", "Unexpected X-Frame-Options values may not provide reliable clickjacking protection.", ""],
                [115, "값을 검토하고 SAMEORIGIN/DENY 또는 CSP frame-ancestors 사용을 권장합니다.", "Review the value and prefer SAMEORIGIN/DENY or CSP frame-ancestors.", ""],
                [116, "먼저 Content-Security-Policy-Report-Only로 적용해 위반 내역을 확인한 뒤, 안정화 후 강제 정책으로 전환하십시오. 예: default-src 'self'; object-src 'none'; base-uri 'self'; frame-ancestors 'self';", "Start with Content-Security-Policy-Report-Only, then enforce after checking violations. Example: default-src 'self'; object-src 'none'; base-uri 'self'; frame-ancestors 'self';", ""],
                [117, "Report-Only CSP가 존재합니다. 위반 보고서를 검토하고 안정화되면 Content-Security-Policy로 전환하십시오.", "A Report-Only CSP exists. Review violation reports and promote it to Content-Security-Policy when stable.", ""],
                [118, "CSP가 없으면 XSS, 허가되지 않은 리소스 로딩, 클릭재킹에 대한 방어가 약해집니다.", "Missing CSP weakens protection against XSS, unauthorized resource loading, and clickjacking.", ""],
                [119, "frame-ancestors 'self' 또는 frame-ancestors 'none' 추가", "Add frame-ancestors 'self' or frame-ancestors 'none'", ""],
                [120, "CSP는 존재하지만 frame-ancestors가 없으면 임베딩 방어가 불완전합니다.", "CSP exists, but embedding protection is incomplete without frame-ancestors.", ""],
                [121, "필요한 iframe 사용 사례를 확인한 뒤 기존 CSP에 frame-ancestors를 추가하십시오.", "Add frame-ancestors to the existing CSP after checking required iframe use cases.", ""],
                [122, "HSTS가 없으면 이후 방문에서도 리다이렉트 전 HTTP 접속 시도가 발생할 수 있습니다.", "Without HSTS, future visits can still attempt plain HTTP before redirecting.", ""],
                [123, "HTTPS, HTTP->HTTPS 리다이렉트, 하위 도메인 HTTPS 상태가 안정적인지 확인한 뒤 적용하십시오.", "Add HSTS only after HTTPS, redirects, and subdomains are confirmed stable.", ""],
                [124, "외부 사이트로 이동할 때 Referer 헤더를 통해 전체 URL이 노출될 수 있습니다.", "Full URLs may leak to external sites through the Referer header.", ""],
                [125, "Nginx/OpenResty 설정에 추가: add_header Referrer-Policy \"strict-origin-when-cross-origin\" always;", "Nginx/OpenResty: add_header Referrer-Policy \"strict-origin-when-cross-origin\" always;", ""],
                [126, "제한하지 않으면 카메라, 마이크, 위치정보, 결제 등 브라우저 기능이 페이지 또는 삽입 콘텐츠에서 사용 가능할 수 있습니다.", "Browser features remain available to the page or embedded content unless restricted.", ""],
                [127, "Nginx/OpenResty 설정에 추가: add_header Permissions-Policy \"camera=(), microphone=(), geolocation=(), payment=()\" always;", "Nginx/OpenResty: add_header Permissions-Policy \"camera=(), microphone=(), geolocation=(), payment=()\" always;", ""],
                [128, "민감 정보 페이지: no-store, no-cache, must-revalidate", "For sensitive pages: no-store, no-cache, must-revalidate", ""],
                [129, "일부 민감 콘텐츠가 브라우저 또는 프록시에 저장될 수 있습니다.", "Some sensitive content may still be stored by browsers or proxies.", ""],
                [130, "개인정보, 계정정보, 기밀정보가 포함된 페이지에는 no-store 적용을 검토하십시오.", "Use no-store for pages containing personal, account, or confidential data.", ""],
                [131, "명시적인 Cache-Control 지시자 사용 권장", "Prefer explicit Cache-Control directives", ""],
                [132, "Expires: 0은 최신 Cache-Control 정책보다 의미가 덜 명확합니다.", "Expires: 0 is less explicit than modern Cache-Control policies.", ""],
                [133, "캐시 정책은 Cache-Control을 중심으로 정의하십시오.", "Use Cache-Control as the primary cache policy.", ""]
            ]
...
def confirm_broken_link(url, status_code):
    ...
    for verify_url in build_broken_link_verify_urls(url, status_code):
    ...
    if keyword_path in url:     ## default "/"
        ...
        #######################################################
           ...
           ## print("check_string2", len(texts))
            ...
            lxw_text = lxw if lxw is not None else "(not assigned)"
            print("occured a erro on keyword exctract procedure 1017", texts[:50], "===>lxw", lxw_text, "error:", exc)
...
def collect_security_header_recommendations(headers, lang=None):
    recommendations = []
    use_korean = (rptLang if lang is None else lang) == 1

    def text(korean, english):
        return korean if use_korean else english
    ...
    csp_report_only = get_header_value(headers, "Content-Security-Policy-Report-Only")
    ...
    def add(header, severity, current, recommended, risk, action, verify):
        recommendations.append({
            "header": header,
            "severity": severity,
            "current": current or text("누락", "missing"),
    ...
    if not x_xss:
        add(
            "X-XSS-Protection",
            text("낮음/레거시", "Low/Legacy"),
            x_xss,
            text(
                "강한 Content-Security-Policy 적용을 우선 권장합니다. 레거시 브라우저 지원 정책이 필요할 때만 '0' 또는 '1; mode=block' 적용을 검토하십시오.",
                "Prefer a strong Content-Security-Policy. If legacy support is required, use '0' or '1; mode=block' by policy.",
            ),
            text(
                "이 헤더는 최신 브라우저에서 권장되지 않으며, XSS 방어의 주 수단이 아닙니다.",
                "This header is deprecated and not the primary XSS defense in modern browsers.",
            ),
            text(
                "주요 조치로 보지 말고 CSP를 우선 적용하십시오. 필요 시 레거시 브라우저 정책용으로만 X-XSS-Protection을 추가하십시오.",
                "Do not treat this as the main fix. Prioritize CSP; optionally add X-XSS-Protection for legacy browser policy only.",
            ),
            "curl -I https://<domain> | findstr /I X-XSS-Protection",
        )
        ...
        add(
            ...
            x_xss,
            text(
                "의도된 설정인지 확인하고, 강한 CSP가 함께 적용되어 있는지 점검하십시오.",
                "Confirm this is intentional and backed by a strong CSP.",
            ),
            text(
                "레거시 XSS 필터링이 비활성화되어 있습니다. 일반적으로 CSP가 충분히 구성된 경우에만 허용 가능한 설정입니다.",
                "Legacy XSS filtering is disabled. This is usually acceptable only when CSP is configured.",
            ),
            text(
                "Content-Security-Policy가 존재하고 충분히 제한적인지 확인하십시오.",
                "Check whether Content-Security-Policy is present and sufficiently restrictive.",
            ),
            "curl -I https://<domain> | findstr /I \"X-XSS-Protection Content-Security-Policy\"",
        )
    ...
    if x_content_type.lower() != "nosniff":
        add(
            ...
            "nosniff",
            text(
                "브라우저가 MIME 타입을 임의로 추론하여 콘텐츠를 의도하지 않은 방식으로 실행할 수 있습니다.",
                "Browsers may infer a different MIME type and execute content in an unintended context.",
            ),
            text(
                "Nginx/OpenResty 설정에 추가: add_header X-Content-Type-Options \"nosniff\" always;",
                "Nginx/OpenResty: add_header X-Content-Type-Options \"nosniff\" always;",
            ),
            "curl -I https://<domain> | findstr /I X-Content-Type-Options",
        )
    ...
    if not x_frame:
        add(
            ...
            x_frame,
            text(
                "SAMEORIGIN 또는 DENY 적용. 가능하면 CSP frame-ancestors도 함께 적용",
                "SAMEORIGIN or DENY; preferably also CSP frame-ancestors",
            ),
            text(
                "다른 사이트가 해당 페이지를 frame/iframe으로 삽입할 수 있어 클릭재킹 위험이 증가합니다.",
                "Other sites may embed the page in a frame, increasing clickjacking risk.",
            ),
            text(
                "Nginx/OpenResty 설정에 추가: add_header X-Frame-Options \"SAMEORIGIN\" always;",
                "Nginx/OpenResty: add_header X-Frame-Options \"SAMEORIGIN\" always;",
            ),
            "curl -I https://<domain> | findstr /I X-Frame-Options",
        )
        ...
        add(
            ...
            "SAMEORIGIN or DENY",
            text(
                "일반적이지 않은 X-Frame-Options 값은 클릭재킹 방어를 안정적으로 제공하지 못할 수 있습니다.",
                "Unexpected X-Frame-Options values may not provide reliable clickjacking protection.",
            ),
            text(
                "값을 검토하고 SAMEORIGIN/DENY 또는 CSP frame-ancestors 사용을 권장합니다.",
                "Review the value and prefer SAMEORIGIN/DENY or CSP frame-ancestors.",
            ),
            "curl -I https://<domain> | findstr /I X-Frame-Options",
        )
    ...
    if not csp:
        csp_action = (
            text(
                "먼저 Content-Security-Policy-Report-Only로 적용해 위반 내역을 확인한 뒤, 안정화 후 강제 정책으로 전환하십시오. "
                "예: default-src 'self'; object-src 'none'; base-uri 'self'; frame-ancestors 'self';",
                "Start with Content-Security-Policy-Report-Only, then enforce after checking violations. "
                "Example: default-src 'self'; object-src 'none'; base-uri 'self'; frame-ancestors 'self';",
            )
        )
        if csp_report_only:
            csp_action = text(
                "Report-Only CSP가 존재합니다. 위반 보고서를 검토하고 안정화되면 Content-Security-Policy로 전환하십시오.",
                "A Report-Only CSP exists. Review violation reports and promote it to Content-Security-Policy when stable.",
            )
        add(
            ...
            text(
                "CSP가 없으면 XSS, 허가되지 않은 리소스 로딩, 클릭재킹에 대한 방어가 약해집니다.",
                "Missing CSP weakens protection against XSS, unauthorized resource loading, and clickjacking.",
            ),
            csp_action,
            "curl -I https://<domain> | findstr /I Content-Security-Policy",
        )
        ...
        add(
            ...
            csp,
            text(
                "frame-ancestors 'self' 또는 frame-ancestors 'none' 추가",
                "Add frame-ancestors 'self' or frame-ancestors 'none'",
            ),
            text(
                "CSP는 존재하지만 frame-ancestors가 없으면 임베딩 방어가 불완전합니다.",
                "CSP exists, but embedding protection is incomplete without frame-ancestors.",
            ),
            text(
                "필요한 iframe 사용 사례를 확인한 뒤 기존 CSP에 frame-ancestors를 추가하십시오.",
                "Add frame-ancestors to the existing CSP after checking required iframe use cases.",
            ),
            "curl -I https://<domain> | findstr /I Content-Security-Policy",
        )
    ...
    if not hsts:
        add(
            ...
            text("조건부", "Conditional"),
            hsts,
            "max-age=31536000; includeSubDomains",
            text(
                "HSTS가 없으면 이후 방문에서도 리다이렉트 전 HTTP 접속 시도가 발생할 수 있습니다.",
                "Without HSTS, future visits can still attempt plain HTTP before redirecting.",
            ),
            text(
                "HTTPS, HTTP->HTTPS 리다이렉트, 하위 도메인 HTTPS 상태가 안정적인지 확인한 뒤 적용하십시오.",
                "Add HSTS only after HTTPS, redirects, and subdomains are confirmed stable.",
            ),
            "curl -I https://<domain> | findstr /I Strict-Transport-Security",
        )
    ...
    if not referrer:
        add(
            ...
            text(
                "외부 사이트로 이동할 때 Referer 헤더를 통해 전체 URL이 노출될 수 있습니다.",
                "Full URLs may leak to external sites through the Referer header.",
            ),
            text(
                "Nginx/OpenResty 설정에 추가: add_header Referrer-Policy \"strict-origin-when-cross-origin\" always;",
                "Nginx/OpenResty: add_header Referrer-Policy \"strict-origin-when-cross-origin\" always;",
            ),
            "curl -I https://<domain> | findstr /I Referrer-Policy",
        )
    ...
    if not permissions:
        add(
            ...
            text(
                "제한하지 않으면 카메라, 마이크, 위치정보, 결제 등 브라우저 기능이 페이지 또는 삽입 콘텐츠에서 사용 가능할 수 있습니다.",
                "Browser features remain available to the page or embedded content unless restricted.",
            ),
            text(
                "Nginx/OpenResty 설정에 추가: add_header Permissions-Policy \"camera=(), microphone=(), geolocation=(), payment=()\" always;",
                "Nginx/OpenResty: add_header Permissions-Policy \"camera=(), microphone=(), geolocation=(), payment=()\" always;",
            ),
            "curl -I https://<domain> | findstr /I Permissions-Policy",
        )
    ...
    if cache_control and (
        ...
        add(
            ...
            cache_control,
            text(
                "민감 정보 페이지: no-store, no-cache, must-revalidate",
                "For sensitive pages: no-store, no-cache, must-revalidate",
            ),
            text(
                "일부 민감 콘텐츠가 브라우저 또는 프록시에 저장될 수 있습니다.",
                "Some sensitive content may still be stored by browsers or proxies.",
            ),
            text(
                "개인정보, 계정정보, 기밀정보가 포함된 페이지에는 no-store 적용을 검토하십시오.",
                "Use no-store for pages containing personal, account, or confidential data.",
            ),
            "curl -I https://<domain> | findstr /I Cache-Control",
        )
    if expires and expires.strip() == "0":
        add(
            ...
            expires,
            text(
                "명시적인 Cache-Control 지시자 사용 권장",
                "Prefer explicit Cache-Control directives",
            ),
            text(
                "Expires: 0은 최신 Cache-Control 정책보다 의미가 덜 명확합니다.",
                "Expires: 0 is less explicit than modern Cache-Control policies.",
            ),
            text(
                "캐시 정책은 Cache-Control을 중심으로 정의하십시오.",
                "Use Cache-Control as the primary cache policy.",
            ),
            "curl -I https://<domain> | findstr /I \"Expires Cache-Control\"",
        )
...
def collect_security_header_recommendations(headers, lang=None):
    recommendations = []
    active_lang = rptLang if lang is None else lang

    def msg(message_id):
        return MessageList[message_id][active_lang]
    ...
    def add(header, severity_id, current, recommended, risk, action, verify):
        recommendations.append({
            "header": header,
            "severity": msg(severity_id),
            "current": current or msg(97),
            "recommended": recommended,
            "risk": risk,
            "action": action,
            "verify": verify,
...
def append_security_header_recommendations(recommendations, domain):
    if not recommendations:
        return

    security_title = MessageList[91][rptLang]
    label_current = MessageList[92][rptLang]
    label_recommended = MessageList[93][rptLang]
    label_risk = MessageList[94][rptLang]
    label_action = MessageList[95][rptLang]
    label_verify = MessageList[96][rptLang]
    appendix_title = f"부록. {security_title}" if rptLang == 1 else f"Appendix. {security_title}"

    progress_make(2, appendix_title, "")
    for index, recommendation in enumerate(recommendations, start=1):
        progress_make(
            6,
            f" {index}) [{recommendation['severity']}] ",
            recommendation["header"],
        )
        progress_make(6, label_current, recommendation["current"])
        progress_make(6, label_recommended, recommendation["recommended"])
        progress_make(6, label_risk, recommendation["risk"])
        progress_make(6, label_action, recommendation["action"])
        progress_make(6, label_verify, recommendation["verify"].replace("<domain>", domain))
...
if __name__ == "__main__":
    ...
    security_header_recommendations = collect_security_header_recommendations(head_table, rptLang)
    #######################################################
    ...
    #######################################################
    # Appendix: Security header recommendations
    #######################################################
    append_security_header_recommendations(security_header_recommendations, baseUrl)
